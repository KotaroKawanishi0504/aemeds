...
# 4. トラブルシューティング（ヘッダーが真っ白・下線だけの場合）
...
- **対応（方法1）**:
  ...
  2. 1 セクション目：ブランド（ロゴ＋トップへのリンクをボタンブロックで配置）。
  3. 2 セクション目：メニュー（リストブロックで ul > li になるように配置）。
  4. 3 セクション目：ツール（検索・言語などのリンク）。
  5. AEM の配信設定で、該当ページの `.plain.html` が **本文のセクションのみ** を返すようにする（フルドキュメントを返すテンプレートやパイプラインの場合は修正が必要）。
