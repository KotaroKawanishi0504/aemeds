...
const LIST_CLASS = 'card-carousel-list';
const ITEM_CLASS = 'card-carousel-item';
const LINK_CLASS = 'card-carousel-link';
const IMAGE_CLASS = 'card-carousel-image';
const TITLE_CLASS = 'card-carousel-title';
const PAGINATION_CLASS = 'card-carousel-pagination';
const PAGINATION_BULLET_CLASS = 'card-carousel-pagination-bullet';
const MODE_CAROUSEL = 'card-carousel-mode-carousel';
const MODE_GRID = 'card-carousel-mode-grid';
...
function setupCarouselMode(block, list, pagination) {
  ...
  if (block.cardCarouselScrollHandler) {
    list.removeEventListener('scroll', block.cardCarouselScrollHandler);
  }
  const onScroll = () => updatePagination(block, list, pagination);
  list.addEventListener('scroll', onScroll);
  block.cardCarouselScrollHandler = onScroll;
...
function setupGridMode(block, pagination) {
  ...
  if (list && block.cardCarouselScrollHandler) {
    list.removeEventListener('scroll', block.cardCarouselScrollHandler);
    block.cardCarouselScrollHandler = null;
  }
}
...
export default function decorate(block) {
  ...
  block.cardCarouselCleanup = () => {
    mq.removeEventListener('change', onResize);
    window.removeEventListener('resize', onResize);
    if (block.cardCarouselScrollHandler) {
      const listEl = block.querySelector(`.${LIST_CLASS}`);
      if (listEl) listEl.removeEventListener('scroll', block.cardCarouselScrollHandler);
    }
  };
}
